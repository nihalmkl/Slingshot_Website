...
export default function CustomCursor() {
  ...
  useEffect(() => {
    ...
    // When mouse is pressed down, it becomes 'click'
    ...
    const onOver = (e: MouseEvent) => {
      ...
      if (isInput) {
        ...
        // If we are currently dragging, keep it as drag, otherwise default
        setState(s => s === 'drag' ? 'drag' : 'default');
      }
    };

    // Custom event dispatched from HomeSection when dragging starts/stops
  ...
  const RING_SIZE = {
    ...
    view:     60,
    navHover: 36,
    click:    88, // Keep large during click
    text:     4,
    drag:     88, // Keep large during drag
    hidden:   88,
  }[state];

  // We only show the up/down bouncing arrows when actively clicking or dragging
  const IS_DRAGGING_OR_CLICKING = state === 'drag' || state === 'click';

  // Wrapper expands to 140px ONLY when dragging/clicking so the arrows fit outside
  ...
  // Only show "SCROLL" text on default hover (not clicking or dragging)
  const SHOW_TEXT = state === 'default'; 
  const SHOW_VIEW = state === 'view';
  ...
  return (
    <>
      ...
      >
        ...
        {/* ── UP CHEVRON (Only visible during drag/click) ── */}
        {IS_DRAGGING_OR_CLICKING && (
          <div style={{ position: 'absolute', top: 10, left: '50%', transform: 'translateX(-50%)', animation: 'chevronUp 1s ease-in-out infinite', lineHeight: 1 }}>
            <svg width="12" height="8" viewBox="0 0 12 8" fill="none">
              <path d="M1 7L6 2L11 7" stroke="rgba(255,255,255,0.95)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </div>
        )}
        ...
        >
          {/* Rotating SCROLL text (Visible by default) */}
          {SHOW_TEXT && (
          ...
          {/* VIEW label (Visible over links) */}
          {SHOW_VIEW && (
            <span style={{ fontFamily: "'Courier New', Courier, monospace", fontSize: '9px', fontWeight: 700, letterSpacing: '0.3em', textTransform: 'uppercase', color: 'rgba(10,10,10,0.8)', userSelect: 'none' }}>
            </span>
          )}

          {/* Centre dot (Disappears when dragging/clicking to leave the circle completely empty) */}
          <div style={{
        ...
        {/* ── DOWN CHEVRON (Only visible during drag/click) ── */}
        {IS_DRAGGING_OR_CLICKING && (
          <div style={{ position: 'absolute', bottom: 10, left: '50%', transform: 'translateX(-50%)', animation: 'chevronDown 1s ease-in-out infinite', lineHeight: 1 }}>
            <svg width="12" height="8" viewBox="0 0 12 8" fill="none">
              <path d="M1 1L6 6L11 1" stroke="rgba(255,255,255,0.95)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </div>
        )}
      ...
      {/* ── Inner precise dot (Disappears during drag/click) ────────────────── */}
      <div
        ref={dotRef}
        aria-hidden="true"
        style={{
